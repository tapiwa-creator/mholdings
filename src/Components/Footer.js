...
const Modal = ({ isOpen, onClose, title, children }) => {
  ...
  return (
    <div
      onClick={onClose}
      style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:'1.5rem', backdropFilter:'blur(4px)' }}
    >
      <div
        onClick={e => e.stopPropagation()}
        style={{ background:'#111827', borderRadius:'20px', border:'1px solid rgba(201,168,76,0.2)', maxWidth:'520px', width:'100%', overflow:'hidden' }}
      >
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'1.5rem 1.75rem', borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
          <h2 style={{ fontFamily:"'DM Serif Display', serif", fontSize:'1.4rem', fontWeight:700, color:'#ffffff', margin:0 }}>{title}</h2>
          <button
            onClick={onClose}
            style={{ background:'rgba(255,255,255,0.06)', border:'none', borderRadius:'8px', width:'34px', height:'34px', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', color:'rgba(255,255,255,0.5)', transition:'background 0.15s' }}
...
const Footer = () => {
  ...
  const bodyText = (text) => (
    <p style={{ fontSize:'14px', color:'rgba(255,255,255,0.5)', lineHeight:1.75, margin:0 }}>{text}</p>
  );
  ...
  return (
    <>
      ...
      <footer style={{ background:'#0a0f1e', borderTop:'1px solid rgba(201,168,76,0.12)', fontFamily:"'DM Sans', sans-serif" }}>
        <div style={{ maxWidth:'1200px', margin:'0 auto', padding:'2.5rem 1.5rem 1.5rem' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))', gap:'2rem 3rem', marginBottom:'2rem' }}>
            ...
            <div style={{ gridColumn:'span 2' }} className="footer-brand">
              ...
              <p style={{ color:'rgba(255,255,255,0.4)', fontSize:'14px', lineHeight:1.8, maxWidth:'280px', margin:0 }}>
