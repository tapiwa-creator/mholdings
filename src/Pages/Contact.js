  ...
  const contactItems = [
    {
      ...
      bg: 'rgba(37,99,235,0.12)', iconColor: '#60a5fa',
      icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />,
    },
    {
      label: 'Phone', value: '+236 781406806, +263 168 8458', href: 'tel:+236781406806',
      bg: 'rgba(13,148,136,0.12)', iconColor: '#34d399',
      icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />,
    },
    {
      label: 'Address', value: 'Harare, Zimbabwe', href: null,
      bg: 'rgba(201,168,76,0.1)', iconColor: '#c9a84c',
    ...
    <>
      ...
      <style>{`
        ...
        @media (max-width:768px) {
          ...
          .form-panel { border-left:none !important; border-top:1px solid rgba(255,255,255,0.06); }
          .name-grid { grid-template-columns:1fr !important; }
        }
        @media (min-width:769px) {
          .form-panel { border-left:1px solid rgba(255,255,255,0.06) !important; border-top:none; }
        }
      ...
      <div style={{ minHeight: '100vh', background: '#0a0f1e', fontFamily: "'DM Sans', sans-serif", color: '#ffffff', overflowX: 'hidden' }}>
        ...
        <div style={{ width: '100%', margin: '0 auto', padding: 'clamp(2rem,5vw,4rem) clamp(1.5rem,5vw,5rem) clamp(3rem,6vw,5rem)', boxSizing: 'border-box' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
            <h1 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 'clamp(1.8rem,4vw,3rem)', fontWeight: 700, color: '#ffffff', marginBottom: '0.75rem', lineHeight: 1.15 }}>
              Get in Touch
            </h1>
            <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: 'clamp(14px,2vw,16px)', lineHeight: 1.8, maxWidth: '580px', margin: '0 auto' }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.07)', overflow: 'hidden' }}>
            <div className="contact-grid">
              ...
              <div style={{ padding: 'clamp(1.5rem,4vw,3rem)' }}>
                <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 'clamp(1.3rem,2.5vw,1.8rem)', fontWeight: 700, color: '#ffffff', marginBottom: '0.5rem' }}>
                  Contact Information
                </h2>
                <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: 'clamp(13px,1.8vw,15px)', lineHeight: 1.75, marginBottom: '2rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                  {contactItems.map((item, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem' }}>
                      ...
                      <div>
                        <p style={{ fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'rgba(255,255,255,0.25)', marginBottom: '3px' }}>
                          {item.label}
                        </p>
                        {item.href ? (
                          <a href={item.href}
                            style={{ color: 'rgba(255,255,255,0.8)', fontSize: 'clamp(13px,1.8vw,15px)', fontWeight: 500, textDecoration: 'none', transition: 'color 0.2s' }}
                            onMouseEnter={e => e.target.style.color = '#c9a84c'}
                            onMouseLeave={e => e.target.style.color = 'rgba(255,255,255,0.8)'}
                          >{item.value}</a>
                        ) : (
                          <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: 'clamp(13px,1.8vw,15px)', fontWeight: 500, margin: 0 }}>{item.value}</p>
                        )}
                      </div>
                    </div>
                  ))}
              ...
              <div style={{ padding: 'clamp(1.5rem,4vw,3rem)' }} className="form-panel">
                <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 'clamp(1.3rem,2.5vw,1.8rem)', fontWeight: 700, color: '#ffffff', marginBottom: '1.5rem' }}>
                  Send a Message
                </h2>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
                  ...
                  {submitStatus.message && (
                    <div style={{
                      ...
                      background: submitStatus.type === 'success' ? 'rgba(52,211,153,0.08)' : 'rgba(239,68,68,0.08)',
                      color: submitStatus.type === 'success' ? '#34d399' : '#f87171',
                      border: `1px solid ${submitStatus.type === 'success' ? 'rgba(52,211,153,0.2)' : 'rgba(239,68,68,0.2)'}`,
                    }}>
                      {submitStatus.message}
                    </div>
                  )}
