...
const Counter = ({ target, suffix = '', label, delay = 0 }) => {
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <div style={{ color: 'rgba(10,15,30,0.4)', fontSize: '12px', fontWeight: 500, marginTop: '8px', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{label}</div>
    </div>
  );
};
    ...
    <>
      ...
      <style>{`
        ...
        .landing-pillar {
          ...
          background:rgba(10,15,30,0.03); border:1px solid rgba(10,15,30,0.07);
        ...
        .landing-vision-card:hover { border-color:rgba(201,168,76,0.35); transform:translateY(-3px); }
        ...
        .landing-team-card:hover { border-color:rgba(201,168,76,0.35); transform:translateY(-5px); }
      ...
      <div style={{ minHeight: '100vh', background: '#f4f1eb', fontFamily: "'DM Sans', sans-serif", color: '#0a0f1e', overflowX: 'hidden' }}>
        ...
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', paddingTop: '150px', paddingBottom: '100px', paddingLeft: 'clamp(1.5rem,5vw,5rem)', paddingRight: 'clamp(1.5rem,5vw,5rem)', overflow: 'hidden' }}>
          <div style={{ position: 'relative', zIndex: 2, maxWidth: '800px', textAlign: 'center', animation: 'fadeUp 0.9s ease both' }}>
            <h1 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 'clamp(2.4rem,5.5vw,4.2rem)', fontWeight: 700, lineHeight: 1.15, color: '#0a0f1e', marginBottom: '1.25rem' }}>
              Delivering{' '}
              <em style={{ color: '#c9a84c', fontStyle: 'italic' }}>smart</em>
              {' '}in<br />software development and architecture
            </h1>
            <p style={{ color: 'rgba(10,15,30,0.55)', fontSize: 'clamp(16px,2.2vw,18px)', lineHeight: 1.8, maxWidth: '620px', margin: '0 auto 2rem' }}>
        ...
        {/* ── STATS ── */}
        <div style={{ background: 'rgba(10,15,30,0.03)', borderTop: '1px solid rgba(201,168,76,0.18)', borderBottom: '1px solid rgba(201,168,76,0.18)' }}>
          <div style={{ width: '100%', padding: '0 clamp(1.5rem,5vw,5rem)' }}>
            <div className="landing-stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)' }}>
              {[
                ...
                <div key={i} style={{ padding: '3rem 1rem', textAlign: 'center', borderRight: i % 2 === 0 ? '1px solid rgba(201,168,76,0.18)' : 'none', borderBottom: i < 2 ? '1px solid rgba(201,168,76,0.18)' : 'none' }}>
                  <Counter {...stat} />
                </div>
              ))}
            </div>
        ...
        <div style={{ width: '100%', padding: '80px clamp(1.5rem,5vw,5rem) 0', display: 'flex', flexDirection: 'column', gap: '80px', boxSizing: 'border-box' }}>
          ...
          <div className="landing-why-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '60px', alignItems: 'center' }}>
            <div>
              <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 'clamp(1.5rem,3.5vw,2.5rem)', fontWeight: 700, color: '#0a0f1e', marginBottom: '1.5rem', lineHeight: 1.2 }}>
                Your Trusted Partner in Innovation & Design
              </h2>
              <p style={{ color: 'rgba(10,15,30,0.55)', fontSize: 'clamp(14px,2vw,15px)', lineHeight: 1.85, marginBottom: '1rem' }}>
              ...
              <p style={{ color: 'rgba(10,15,30,0.55)', fontSize: 'clamp(14px,2vw,15px)', lineHeight: 1.85 }}>
            ...
            <div style={{ background: '#ffffff', border: '1px solid rgba(201,168,76,0.18)', borderRadius: '20px', padding: '2.5rem', display: 'flex', flexDirection: 'column', gap: '14px', position: 'relative', overflow: 'hidden' }}>
              ...
              {[
                ...
                <div key={i} className="landing-pillar">
                  ...
                  <div>
                    <h4 style={{ fontFamily: "'DM Serif Display', serif", color: '#0a0f1e', fontSize: 'clamp(14px,2vw,16px)', fontWeight: 600, marginBottom: '5px' }}>{p.title}</h4>
                    <p style={{ color: 'rgba(10,15,30,0.45)', fontSize: 'clamp(12px,1.8vw,13px)', lineHeight: 1.6 }}>{p.desc}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
              <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 'clamp(1.5rem,3.5vw,2.5rem)', fontWeight: 700, color: '#0a0f1e', marginBottom: '0.75rem' }}>Our Vision</h2>
              <p style={{ color: 'rgba(10,15,30,0.5)', fontSize: 'clamp(13px,2vw,15px)', lineHeight: 1.75, maxWidth: '580px', margin: '0 auto' }}>
            ...
            <div className="landing-vision-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: '20px' }}>
              {[
                { num: '01', title: 'Smart Blueprint', desc: 'Lead digital transformation in construction, delivering future-ready, intelligent solutions for all.', bg: 'rgba(37,99,235,0.1)', iconColor: '#3b82f6', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /> },
                { num: '02', title: 'Digital Empowerment', desc: 'Drive technology innovation that empowers people, streamlining processes through digitalisation.', bg: 'rgba(13,148,136,0.1)', iconColor: '#0d9488', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /> },
                ...
                { num: '04', title: 'Next-Gen Software', desc: 'Deliver cutting-edge software upgrades that digitise workflows and connect users worldwide.', bg: 'rgba(124,58,237,0.1)', iconColor: '#7c3aed', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /> },
              ].map((card, i) => (
                <div key={i} className="landing-vision-card">
                  ...
                  <h3 style={{ fontSize: 'clamp(14px,2vw,16px)', fontWeight: 700, color: '#0a0f1e', marginBottom: '0.5rem' }}>{card.title}</h3>
                  <p style={{ color: 'rgba(10,15,30,0.45)', fontSize: 'clamp(12px,1.8vw,14px)', lineHeight: 1.75 }}>{card.desc}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ paddingBottom: '0' }}>
            <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
              <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 'clamp(1.5rem,3.5vw,2.5rem)', fontWeight: 700, color: '#0a0f1e', marginBottom: '0.75rem' }}>Our Team</h2>
              <p style={{ color: 'rgba(10,15,30,0.5)', fontSize: 'clamp(13px,2vw,15px)', lineHeight: 1.75, maxWidth: '600px', margin: '0 auto' }}>
            ...
            <div className="landing-team-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '24px' }}>
              {[
                { img: '/1.jpg', name: 'Lennox B Chapata', role: 'Lead Planner', desc: 'Oversees the design, planning, and execution of building projects from concept to completion. Guides architectural teams, ensures creative and functional design, and collaborates with clients and contractors to deliver aesthetically compelling, safe and sustainable structures.' },
                { img: '/2.jpg', name: 'Comfort T Mkunyadze', role: 'Tech Resource Associate', desc: 'Supports the technical teams by identifying and managing the right tools, technologies and resources to drive project success. Collaborates closely with developers and stakeholders to streamline workflows, solve technical challenges, and ensure efficient delivery of high-quality solutions.' },
                { img: '/3.jpg', name: 'Mbonge R', role: 'Technical Lead', desc: 'A technical leader with a strong ability to combine hands-on technical expertise and team guidance to deliver effective solutions. Specializing in overseeing development processes, making key architectural decisions, and supporting team members to achieve project goals efficiently.' },
              ].map((member, i) => (
                <div key={i} className="landing-team-card">
                  <div style={{ width: 'clamp(72px,12vw,100px)', height: 'clamp(72px,12vw,100px)', borderRadius: '50%', overflow: 'hidden', marginBottom: '1.25rem', border: '2px solid rgba(201,168,76,0.35)', outline: '3px solid rgba(201,168,76,0.1)', outlineOffset: '3px' }}>
                    <img src={member.img} alt={member.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  </div>
                  <h3 style={{ fontSize: 'clamp(14px,2vw,16px)', fontWeight: 700, color: '#0a0f1e', marginBottom: '0.4rem' }}>{member.name}</h3>
                  <span style={{ display: 'inline-block', background: 'rgba(201,168,76,0.12)', color: '#8a6d1e', borderRadius: '100px', padding: '4px 14px', fontSize: '12px', fontWeight: 600, marginBottom: '0.85rem', border: '1px solid rgba(201,168,76,0.25)' }}>
                    {member.role}
                  </span>
                  <p style={{ color: 'rgba(10,15,30,0.45)', fontSize: 'clamp(12px,1.8vw,13px)', lineHeight: 1.75 }}>{member.desc}</p>
                </div>
              ))}
            </div>
          </div>
